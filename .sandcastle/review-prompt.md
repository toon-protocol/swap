...
readability, redundant code, comment quality, formatting, small refactors that
preserve behavior.
...
- the diff introduces a functional defect, an unhandled edge case that breaks
  correctness, a security hole, or breaks the gate
- the diff asserts something factually wrong (in code, tests, or docs) that
