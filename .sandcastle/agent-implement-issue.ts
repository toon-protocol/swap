...
//                             a dead credential — see toon-meta#248, swap#92,
//                             and ./mint-app-token.ts. HOST ONLY: the private
//                             key is deliberately absent from
...
// (toon-meta#248 / swap#92, porting connector#462 / connector#463)
// ---------------------------------------------------------------------------
...
try {
  ...
  // PUBLISH EARLY (toon-meta#248 / swap#92, porting connector#462). The
  // implementer has committed; get those commits onto origin NOW rather than
  // after the reviewer. Two reasons:
  //   1. Recoverability — a run that dies during review (timeout,
  //      cancellation, runner death) leaves the completed implementation on a
  //      remote branch instead of losing it with the container. connector
  //      lost three completed implementations exactly this way before this
  //      existed.
  //   2. It is the cheapest moment to fail: if push auth is broken we learn
  //      it here, minutes in, not an hour later.
  ...
  if (autoMerge && !blocking) {
    ...
    // run's total length is irrelevant (toon-meta#248 / swap#92). This also
    // refreshes the host's GH_TOKEN, which the `gh` calls below depend on.
