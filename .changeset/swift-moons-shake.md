...
Operator-facing env/config names (`MILL_MNEMONIC`, `MILL_BLS_PORT`, …) are unchanged. Connector stays at ^3.20.1 (highest published; the connector npm publish pipeline is broken past 3.20.1 — bump to ^3.28 when fixed). The embedded child-connector boot (`relation: 'parent'` skip + `setPacketHandler` seam) is re-verified against the installed connector by a new boot smoke test.
